...
namespace Todo.SoftEng.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("SoftEng") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "SoftEng", DisplayName = "SoftEng API", Resources = { "SoftEng" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["SoftEng_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["SoftEng_Web:RootUrl"]!.EnsureEndsWith('/');

            /* SoftEng_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["SoftEng_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["SoftEng_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["SoftEng_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
        ...
        // Blazor Client
        var blazorClientId = configurationSection["SoftEng_Blazor:ClientId"];
        if (!blazorClientId.IsNullOrWhiteSpace())
        {
            var blazorRootUrl = configurationSection["SoftEng_Blazor:RootUrl"]?.TrimEnd('/');
        ...
        var blazorServerTieredClientId = configurationSection["SoftEng_BlazorServerTiered:ClientId"];
        if (!blazorServerTieredClientId.IsNullOrWhiteSpace())
        {
            var blazorServerTieredRootUrl = configurationSection["SoftEng_BlazorServerTiered:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["SoftEng_BlazorServerTiered:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["SoftEng_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["SoftEng_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        if (!MauiClientId.IsNullOrWhiteSpace())
        {
            ...
            await CreateApplicationAsync(
                ...
                grantTypes: new List<string> { OpenIddictConstants.GrantTypes.AuthorizationCode, },
                scopes: commonScopes,
                redirectUri: $"{MauiRootUrl}/swagger/oauth2-redirect.html",
